            ...
            @foreach ($data[0] as $item)
                <tr>
                    ...
                    @foreach ($data[1] as $esp)
                        @if($esp['id'] == $item['especialidade_id'])
                             <td class="d-none d-md-table-cell">{{ $esp['name'] }}</td>
                        @endif
                    @endforeach
                    <td>
                        ...
                        <a nohref style="cursor:pointer" onclick="showInfoModal('{{ $item['name'] }}', '{{ $item['crmv'] }}')" class="btn btn-primary">
